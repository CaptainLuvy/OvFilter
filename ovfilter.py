...
def load_unikmers(unikmer_file_path):
    ...
    """
    unikmers = set()
    print(f"Loading unique k-mers from {unikmer_file_path}...")
    ...
    print(f"Loaded {len(unikmers)} k-mer hashes.")
    return unikmers
...
def map_reads_to_unikmers(reads_file_path, unikmers, k):
    ...
    """
    read_to_unikmer_map = {}
    print(f"Scanning reads file {reads_file_path}...")
    ...
    format_name = "FASTQ" if is_fastq else "FASTA"
    print(f"Detected Reads format: {format_name}")
    ...
    with open_file(reads_file_path) as handle:
        ...
        for title, seq, *_ in iterator:
            ...
            count += 1
            if count % 10000 == 0:
                print(f"Scanned {count} reads... Hits: {mapped_count}", end='\r')

    print(f"\nProcessing complete. Total Reads: {count}, Reads with Unikmer: {mapped_count}")
    ...
    print("Building compressed index (Memory Optimized)...")
    processed_map = {}
...
def process_chunk(chunk_data):
    ...
    ignore_overhang = args.get('ignore_overhang', False)
    ...
    for row in rows:
        ...
        # 4. Overhang
        if not reason and not ignore_overhang:
...
def process_paf(paf_file, output_file, discarded_file, r2u_map, args):
    """Process PAF file (Supports parallel stream processing, reduces memory usage)"""
    global GLOBAL_R2U_MAP
    print(f"Reading and processing PAF file {paf_file} (stream mode)...")
    
    # Prepare argument dictionary
    args_dict = {
        'keep_contained': args.keep_contained,
        'overhang_tolerance': args.overhang_tolerance,
        'ignore_overhang': args.ignore_overhang,
        'min_align_len': args.min_align_len,
        'min_identity': args.min_identity,
        'min_shared': args.min_shared,
        'tolerance': args.tolerance,
    ...
    if args.output_numeric:
        ...
        print(f"Saving numeric-only overlaps to: {args.output_numeric}")

    f_discard = None
    if discarded_file:
        f_discard = open_file(discarded_file, 'wt') if discarded_file.endswith('.gz') else open(discarded_file, 'w', newline='')
        
    writer = csv.writer(f_out, delimiter='\t')
    discard_writer = csv.writer(f_discard, delimiter='\t') if f_discard else None
    
    stats = {k: 0 for k in ["self", "contained", "short", "identity", "overhang", "no_unikmer", 
                            "unikmer_fail", "unikmer_no_shared", "unikmer_min_shared", "unikmer_tolerance", "unikmer_max_sd"]}
    reason_map = {
        "Self": "self", "Contained": "contained", "Short": "short", 
        "LowIdt": "identity", "Overhang": "overhang", 
        "NoUnikmer": "no_unikmer", 
        "UnikmerFail": "unikmer_fail",
        "UnikmerFail_NoShared": "unikmer_no_shared",
        "UnikmerFail_MinShared": "unikmer_min_shared",
        "UnikmerFail_Tolerance": "unikmer_tolerance",
        "UnikmerFail_MaxSD": "unikmer_max_sd"
    }
    
    kept_count = 0
    total_processed = 0
    
    # Kept Stats Accumulators
    kept_stats = {
        'sum_shared': 0,
        'sum_sd': 0.0,
        'sum_idt': 0.0,
    ...
    def write_results(chunk_res):
        nonlocal kept_count, total_processed
        for r_row, r_reason, is_numeric_pass, stat_shared, stat_sd, idt in chunk_res:
            ...
            if r_reason:
                if r_reason in reason_map:
                    stats[reason_map[r_reason]] += 1
                if discard_writer:
                    r_row.append(r_reason)
                    discard_writer.writerow(r_row)
            else:
                kept_count += 1
                ...
                # Update kept stats
                kept_stats['count'] += 1
                kept_stats['sum_shared'] += stat_shared
                kept_stats['sum_sd'] += stat_sd
                kept_stats['sum_idt'] += idt
    ...
    if args.threads > 1:
        print(f"Starting {args.threads} processes for parallel stream processing...")
        ...
        if is_fork_safe:
            print("Detected Linux/Fork environment: Using shared memory optimization mode")
            pool = multiprocessing.Pool(processes=args.threads)
        else:
            print("Detected Windows/Spawn environment: Using parameter passing mode")
        ...
        try:
            with pool:
                ...
                if tqdm:
                    with tqdm(desc="Processing Overlaps", unit="chunk") as pbar:
            ...
            print(f"\nParallel processing error: {e}")
            raise e
            
    else:
        print("Using single-threaded stream processing...")
        GLOBAL_R2U_MAP = r2u_map
        chunk_size = 5000
        batch_gen = batch_reader(paf_file, chunk_size, args_dict)
        if tqdm:
    ...
    if f_discard: f_discard.close()

    # --- Output statistics summary file ---
    stats_file = output_file + ".stats.txt"
    try:
        with open(stats_file, 'w') as f_stats:
            f_stats.write("=== Ovfilter Statistics ===\n")
            f_stats.write(f"Total Processed: {total_processed}\n")
            
            if total_processed > 0:
                kept_pct = kept_count / total_processed * 100
            else:
                kept_pct = 0.0
                
            f_stats.write(f"Kept Overlaps:   {kept_count} ({kept_pct:.2f}%)\n")
            f_stats.write(f"Discarded:       {total_processed - kept_count}\n\n")
            f_stats.write("--- Discard Reasons ---\n")
            
            # Define order of reasons to match processing pipeline
            ordered_reasons = [
                "self", 
                "contained", 
                "short", 
            ...
            for reason in ordered_reasons:
                ...
                if total_processed > 0:
                    ...
                    pct = 0.0
                f_stats.write(f"{reason:<20}: {count:<12} ({pct:.2f}%)\n")

            # Aggregate Unikmer Failures
            unikmer_sub_reasons = ["unikmer_no_shared", "unikmer_min_shared", "unikmer_tolerance", "unikmer_max_sd", "unikmer_fail"]
            total_unikmer_fail = sum(stats.get(r, 0) for r in unikmer_sub_reasons)
            
            if total_processed > 0:
                u_pct = total_unikmer_fail / total_processed * 100
            else:
                u_pct = 0.0
            
            f_stats.write(f"{'unikmer_total':<20}: {total_unikmer_fail:<12} ({u_pct:.2f}%)\n")
            
            # Detailed breakdown
            display_map = {
                "unikmer_no_shared": "no_shared",
                "unikmer_min_shared": "min_shared",
                "unikmer_tolerance": "tolerance",
                "unikmer_max_sd": "max_sd",
                "unikmer_fail": "unknown"
            }
            
            for reason in unikmer_sub_reasons:
                count = stats.get(reason, 0)
                # Show all even if 0, to make it clear which params are tracked
                if total_processed > 0:
                    pct = count / total_processed * 100
                else:
                    pct = 0.0
                
                display_name = display_map.get(reason, reason)
                f_stats.write(f"  - {display_name:<16}: {count:<12} ({pct:.2f}%)\n")
            
            # Print any other reasons that might be in stats but not in our ordered list (just in case)
            processed_reasons = set(ordered_reasons) | set(unikmer_sub_reasons)
            for reason, count in stats.items():
                if reason not in processed_reasons and count > 0:
                     if total_processed > 0:
                        pct = count / total_processed * 100
                     else:
                        pct = 0.0
        ...
        # --- Append Kept Stats ---
        if kept_stats['count'] > 0:
            avg_shared = kept_stats['sum_shared'] / kept_stats['count']
            avg_sd = kept_stats['sum_sd'] / kept_stats['count']
            avg_idt = kept_stats['sum_idt'] / kept_stats['count'] * 100
            
            with open(stats_file, 'a') as f_stats:
                f_stats.write("\n--- Kept Overlaps Quality ---\n")
                f_stats.write(f"{'Avg Identity':<20}: {avg_idt:.2f}%\n")
                f_stats.write(f"{'Avg Shared Unikmers':<20}: {avg_shared:.2f}\n")
                f_stats.write(f"{'Avg SD':<20}: {avg_sd:.2f}\n")
            print(f"Kept stats appended to: {stats_file}")
            
    except Exception as e:
        print(f"Failed to write statistics file: {e}")

    print(f"\n\n=== Filtering Complete ===")
    print(f"Total Processed: {total_processed}, Kept: {kept_count}")
    print(f"Discard details: {stats}")
...
def main():
    ...
    parser.add_argument("-d", "--discarded", help="[Optional] Discarded overlaps file")
    ...
    parser.add_argument("--ignore-overhang", action="store_true", help="Skip overhang checking entirely")
    ...
    if args.numeric_only:
        print("Numeric Only mode: Skipping Unikmer loading and mapping.")
        r2u_map = {}
    else:
    ...
    process_paf(args.paf, args.output, args.discarded, r2u_map, args)
